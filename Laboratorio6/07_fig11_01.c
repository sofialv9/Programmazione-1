...
int main(void){
   ...
   FILE *cfPtr = NULL; // cfPtr = clients.txt file pointer - struct FILE definita in stdio.h

   // fopen opens the file. Exit the program if unable to create the file 
   if ((cfPtr = fopen("clients.txt", "w")) == NULL) { // Apriamo il file in scrittura (flag "w") - stringa che indica il percorso al file (in questo caso relativo, cioè il file si trova nella stessa cartella) - fopen restituisce un puntatore al file se questo viene aperto con successo, altrimenti restituisce NULL
      ...
      scanf("%d%s%lf", &account, name, &balance); // formato: 1234 John 12.5 - per name non serve la & perché n è già un puntatore essendo un array
