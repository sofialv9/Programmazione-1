...
char *generateString(int l) {
    ...
    char *str = calloc(l+1,sizeof(char)); //ERRORE-> non ho fatto l'allocazione dinamica
...
//ERRORE-> messo il valore val prima del ciclo e non al suo interno, nella condizione del while ho dimenticato il strcmp e ho fatto il confronto tra le stringhe sbagliate
void sortArray(char **A, int n) {
    // scorri tutte le sottosequenze ordinate che iniziano con zero 
    for(int i = 1; i < n; i++) {
        // data la sottosequenza che inizia in zero e finisce in i-1, l'obiettivo è di inserire il valore a[i] nella posizione corretta della sottosequenza ordinata
...
char* concatString(char **A, int n) {
    // crea una stringa lunga n * 12 con n numero di stringhe contenute nell'array A
    char* outstr = calloc(n * 12, sizeof(char)); //ERRORE-> dimenticato la lunghezza da inserire
