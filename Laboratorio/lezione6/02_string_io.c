...
int main() {
    ...
    // notiamo che il metodo sopra si "fermerà" quando incontra lo spazio - se scrivo "Hello World" stampa solo "Hello"
    // i caratteri non inseriti in string sono "in attesa" nel buffer dello standard input
    ...
    while(getchar()!='\n'); // legge un carattere, controlla che sia diverso da quello di terminazione e continua finchè non lo incontra - scarta tutti i caratteri fino a quello di terminazione

    // METODO 2: fgets
    printf("Inserisci una riga di testo: ");
    // Con fgets possiamo specificare quanti caratteri leggere
    // e da quale file leggerli (stdin indica lo standard input - la tastiera)
    // leggiamo 9 caratteri (qualsiasi tipo di carattere) dalla standard input stdin (la decima posizione è usata per il carattere di terminazione) e li mettiamo in string
    ...
    while(getchar()!='\n'); // rimmuoviamo i caratteri in eccesso (dopo il nono) da stdin

    // METODO 3: getchar
    // Possiamo leggere i caratteri "manualmente" fino al massimo di caratteri che possiamo inserire nella stringa - legge una stringa di lunghezza fino a MAXLEN - 1 (254)
    printf("Inserisci una riga di testo: ");
    int i = 0;
    char c;
    while((c = getchar()) != '\n' && i < MAXLEN-1)
