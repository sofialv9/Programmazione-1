...
int main(void) {
   ...
   /*
   tabella di traccia che mostra come le variabili cambiano nel tempo
   ...
   */
   ...
   // processing phase
   while (counter <= 10) { // loop 10 times - si sarebbe potuto usare un ciclo for poichè si conosce già il numero di iterazioni
   printf("%s", "Enter grade: "); // prompt for input - si poteva scrivere printf("Enter grade: ") o puts("Enter grade: ")
      int grade = 0; // grade value - dichiarata all'interno del while (variabile locale)
      scanf("%d", &grade); // read grade from user - la & permette di accedere a grade in scrittura
      total = total + grade; // add grade to total - oppure total += grade
