...
// fig03_04.c
// Class-average program with sentinel-controlled iteration - si può inserire una qualsiasi quantità di voti
#include <stdio.h>
...
int main(void) {
   ...
   while (grade != -1) {
      ...
      printf("%s", "Enter grade, -1 to end: "); // prompt for input
      scanf("%d", &grade); // read next grade - se non ci fossero queste due istruzioni il ciclo sarebbe infinito
   } // end while
   ...
   if (counter != 0) {
      ...
      double average = (double) total / counter; // avoid truncation - casting per convertire uno degli operandi al formato in virgola mobile
      // oppure: total / (double) counter ; total / (counter * 1.0)

      // (char*) / (char*) - errore in fase di compilazione (non si può fare la divisione tra stringhe)
